...
export class ShareLinkManager {
  ...
  parseInitialHash() {
    ...
    // Check URL search parameters first (e.g. /explore?lat=...&lng=...)
    if (typeof window !== 'undefined' && window.location?.search && window.location.search.length > 1) {
      const searchParams = new URLSearchParams(window.location.search);
      if (searchParams.has('lat') && (searchParams.has('lon') || searchParams.has('lng'))) {
        params = searchParams;
        isQuery = true;
      }
    }

    // Fall back to URL hash (e.g. #lat=...&lon=...)
    if (!params) {
    ...
    // Reject out-of-range latitude/longitude (-90 to 90, -180 to 180)
    if (lat < -90 || lat > 90 || lon < -180 || lon > 180) {
      console.warn(`[TRINETRA] Out-of-bounds coordinates: lat=${lat}, lon=${lon}. Ignoring.`);
      return null;
    }
    ...
    const targetMarker = params.get('source') === 'satquery' || params.has('name') || isQuery
      ? {
          name: params.get('name') || params.get('label') || 'SatQuery Analysis Target',
          source: params.get('source') || 'satquery',
          imageId: params.get('imageId') || null,
          modality: params.get('modality') || null,
          date: params.get('date') || null,
          bbox: params.get('bbox') || null,
        }
      : null;

    const defaultAlt = isQuery ? 5000 : 800;
    let alt = parseOr(params.get('alt') ?? params.get('height'), defaultAlt);
    if (params.has('zoom') && !params.has('alt') && !params.has('height')) {
      const zoom = parseOr(params.get('zoom'), 14);
      alt = Math.max(200, Math.min(20000000, 35200000 / Math.pow(2, zoom)));
    }
    alt = Math.max(100, Math.min(25000000, alt));
  ...
  async applyState(state, { applyCamera = true, navigationToken = null } = {}) {
    ...
    // Render tactical target marker and footprint if arriving with target location
    if (state.targetMarker && this.viewer?.entities) {
      this._renderTargetMarker(state);
    }
    const view = {
  ...
  _updateHash() {
    ...
    history.replaceState(null, '', `${path}${search}#${params.toString()}`);
  }
  ...
  /**
   * Render a tactical target marker and optional bounding box footprint on the Cesium globe.
   */
  _renderTargetMarker(state) {
    if (!this.viewer?.entities || !state.targetMarker) return;
    try {
      const markerId = 'satquery-analysis-marker';
      const footprintId = 'satquery-analysis-footprint';

      const existingMarker = this.viewer.entities.getById(markerId);
      if (existingMarker) this.viewer.entities.remove(existingMarker);

      const existingFootprint = this.viewer.entities.getById(footprintId);
      if (existingFootprint) this.viewer.entities.remove(existingFootprint);

      const targetPos = Cesium.Cartesian3.fromDegrees(state.lon, state.lat, 0);
      const targetName = state.targetMarker.name || 'SatQuery Analysis Target';
      ...
      this.viewer.entities.add({
        ...
        label: {
          text: `⦿ ${targetName}\n[SatQuery AI Target]`,
          font: 'bold 12px "JetBrains Mono", monospace, sans-serif',
          ...
          disableDepthTestDistance: Number.POSITIVE_INFINITY,
          backgroundColor: Cesium.Color.fromCssColorString('rgba(7, 16, 21, 0.85)'),
          showBackground: true,
          backgroundPadding: new Cesium.Cartesian2(8, 4),
        },
      });

      if (state.targetMarker.bbox) {
        const parts = state.targetMarker.bbox.split(',').map(Number);
        if (parts.length === 4 && parts.every(Number.isFinite)) {
          const [minX, minY, maxX, maxY] = parts;
          this.viewer.entities.add({
            id: footprintId,
            name: `${targetName} Footprint`,
            rectangle: {
              ...
              outline: true,
              outlineColor: Cesium.Color.fromCssColorString('#00f0ff').withAlpha(0.75),
              outlineWidth: 2,
              height: 0,
              heightReference: Cesium.HeightReference.CLAMP_TO_GROUND,
            },
      ...
      console.warn('[TRINETRA] Failed to render target marker entity:', err);
    }
  }
