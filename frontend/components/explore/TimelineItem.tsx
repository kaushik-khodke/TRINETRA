  ...
  return (
    <button
      ...
      className={`group relative flex flex-col items-center p-2 rounded-lg transition-all min-w-[76px] ${
        isSelected
          ? "bg-orange-500/20 border-2 border-orange-400 shadow-lg shadow-orange-500/30 -translate-y-1"
    ...
    >
      ...
      {/* Date */}
      <span className="font-semibold text-xs text-white group-hover:text-orange-200 tracking-tight">
        {dateStr}
      </span>
      <span className="text-[10px] font-mono text-slate-400">{yearStr}</span>

      {/* Platform Chip */}
      <span className="mt-1 text-[9px] font-mono uppercase bg-black/50 px-1 rounded text-orange-300/80 border border-white/5">
      ...
      <div
        className={`absolute -bottom-1.5 w-2 h-2 rotate-45 transition-colors ${
          isSelected ? "bg-orange-400" : "bg-transparent"
        }`}
      />
    </button>
  )
