...
export const getDemoResult = (request: AnalysisRequest): AnalysisResponse => { const scenario = demoScenarios.find((item) => item.mode === request.mode && item.query.toLowerCase() === request.query.toLowerCase()) ?? demoScenarios.find((item) => item.mode === request.mode) ?? demoScenarios[0]; const rawUrl = request.images[0]?.url || (request.mode === "temporal" ? imagePresets.before.url : imagePresets.optical.url); const overlayUrl = request.images[1]?.url || (request.mode === "temporal" ? imagePresets.after.url : request.mode === "fusion" ? imagePresets.sar.url : rawUrl); return { id: `analysis-${Date.now()}`, mode: request.mode, query: request.query, answer: scenario.response, confidence: request.mode === "fusion" ? "medium" : "high", confidenceScore: request.mode === "fusion" ? 0.74 : 0.91, evidence: ["Connected evidence region 01", "Cross-checked spatial pattern", "Compared imagery metadata"], annotations: [{ label: "Primary evidence", x: 18, y: 28, width: 26, height: 22, color: "cyan" }, { label: "Change region", x: 61, y: 42, width: 24, height: 26, color: "amber" }], steps: [{ label: "Images loaded", detail: `${request.images.length} inputs verified`, duration: "0.2s", status: "complete" }, { label: "Imagery aligned", detail: "Normalizing contrast and spatial context", duration: "1.1s", status: "complete" }, { label: "Visual evidence mapped", detail: "Finding regions relevant to your question", duration: "2.4s", status: "complete" }, { label: "Answer composed", detail: "Citing observable evidence", duration: "0.8s", status: "complete" }], model: "SatQuery Vision v0.9", processingTime: "4.5s", resolution: "10 m / pixel", imageType: request.mode === "fusion" ? "Sentinel-2 MSI + Sentinel-1 SAR" : "Sentinel-2 MSI", createdAt: new Date().toISOString(), images: request.images, rawImageUrl: rawUrl, overlayImageUrl: overlayUrl } }
  ...
  submitAnalysis: async (request: AnalysisRequest): Promise<AnalysisResponse> => {
    ...
    if (hasRealFiles) {
      try {
        ...
        console.warn("[analysisAPI] Real API call error:", err);
        throw err;
      }
    }
