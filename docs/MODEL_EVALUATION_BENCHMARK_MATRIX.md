...
| **DIOR-RSVG Region Grounding** *(Balanced Profile)* | `2026-09-14` | **DIOR-RSVG**<br>0.5m–1.0m VHR Optical Images (4,000 test queries) | `VisualGroundingEncoder`<br>(10 Epochs, Bounding Box Head) | **Mean IoU**: `0.0002` (0.02%)<br>**Recall@0.50**: `0.00%` | **Mean IoU (mIoU)**: `0.1965` (19.65%)<br>**Median IoU**: `0.1007`<br>**Recall@0.50**: `13.93%`<br>**Recall@0.75**: `2.00%` | **TransVG**: `mIoU = 28.42%`, `Recall@0.5 = 26.15%`<br>**RefFM**: `mIoU = 32.50%` | Zhan Yang et al. (2023), *Referring Remote Sensing Image Object Grounding*, **IEEE TGRS** | <span style="color:green">**PASS (+19.63% mIoU Gain)**</span> |
...
- **Result**: Baseline network has zero localization ability (`mIoU = 0.02%`, `Recall@0.5 = 0.00%`). Trained specialist reaches **19.65% mIoU** and **13.93% Recall@0.50** in 10 epochs.
- **Literature Comparison**: TransVG achieves 28.42% mIoU; TRINETRA's lightweight head provides an effective starting specialist that can be enhanced via additional quality epochs without changing API contracts.
